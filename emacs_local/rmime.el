;Here's what I have in my ".emacs" file for rmime:
;
;---------<snippet>
      ;; rmime/metamail stuff
...
;      (setq rmime-mimencode-program "mmencode")
;
;---------</snippet>
;
;-- 
;Hal DeVore              | PGP Key fingerprint:
;BMC Software, Inc.      | EA A3 2F 62 98 38 B0 15  F0 85 76 33 E1 5A 2A 64
;Houston, TX  USA        | 
;
;---------------------rmime.el begin
...
(defvar rmime-uufilter-program "ccdecode"
...
;;; Mouse stuff, by TRS.  
;;; Scan down each line in the region, and highlight it seperately
;;; Silly regex stuff so we only highlight the word.  Silly
(defun rmime-highlight-mouse-region (beg end)
  (save-excursion
    (goto-char beg)
    (while (< (point) end)
      (beginning-of-line)
...
(defun rmime-digest-toc-1 (body)
  (let ((here   (point))
	...
	(rmime-highlight-mouse-region here (point))
...
(defun rmime-application/octet-stream (content type encoding disposition description)
  (let ((here   (point))
    ...
    (insert rmime-leader (or description "Click here to receive file transmission") "\n")
...
(defun rmime-*/* (content type encoding disposition description)
  (let ((here   (point))
    ...
    (prog1
	(cons (list (point-marker) content type encoding) nil)
      (insert rmime-leader (or description (format "Click here for \"%s\" data" type)) "\n")
...
(defun rmime-action (content type encoding)
  (let ((filename (make-temp-name "/tmp/metamail")))
    ...
    (call-process rmime-metamail-program nil 0 nil "-b" "-x" "-z" "-m" "emacs" "-c" type "-E" encoding filename)))
...
(defun rmime-insert-content (binaryp encoding content)
  (cond ((rmime-member-string= encoding rmime-transparent-encodings)
	...
	((and rmime-uufilter-program  (string= encoding "x-uuencode"))
	 (rmime-decode-content rmime-uufilter-program  '("-o")                           content))
...
(defun rmime-decode-content (program args content)
  (let ((filename (make-temp-name "/tmp/mime")))
	...
	(progn (rmime-write-content filename content)
	       (or (eq (apply 'call-process program filename
			      (list t "/dev/null") nil args) 0)
...
(defun rmime-interact (keymap function &rest arguments)
  (let ((action (make-symbol "rmime-interaction")))
    ...
    (define-key keymap [mouse-3] action)))
